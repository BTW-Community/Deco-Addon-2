    ...
    public boolean onPlayerDestroyBlock(int par1, int par2, int par3, int par4)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (!this.currentGameType.isCreative() && !this.mc.thePlayer.canHarvestBlock(var6, par1, par2, par3) && var6.CanConvertBlock(this.mc.thePlayer.getCurrentEquippedItem(), var5, par1, par2, par3))
                {
        ...
        if (!this.mc.thePlayer.isUsingItem())
        {
            if (!this.currentGameType.isAdventure() || this.mc.thePlayer.canCurrentToolHarvestBlock(par1, par2, par3))
            {
                ...
                else if (!this.isHittingBlock || !this.sameToolAndBlock(par1, par2, par3))
                {
                    ...
                    float var5 = this.mc.thePlayer.UpdateMiningSpeedModifier();
                    this.netClientHandler.addToSendQueue(new FCPacket166StartBlockHarvest(par1, par2, par3, par4, var5));
                    int var6 = this.mc.theWorld.getBlockId(par1, par2, par3);

                    if (var6 > 0 && this.curBlockDamageMP == 0.0F)
                    {
                        Block.blocksList[var6].onBlockClicked(this.mc.theWorld, par1, par2, par3, this.mc.thePlayer);
                    }

                    if (var6 > 0 && Block.blocksList[var6].getPlayerRelativeBlockHardness(this.mc.thePlayer, this.mc.thePlayer.worldObj, par1, par2, par3) >= 1.0F)
                    {
        ...
        if (this.blockHitDelay <= 0)
        {
            ...
            else
            {
                ...
                if (this.sameToolAndBlock(par1, par2, par3))
                {
                    ...
                    {
                        this.mc.sndManager.playSound(var6.stepSound.getStepSound(), (float)par1 + 0.5F, (float)par2 + 0.5F, (float)par3 + 0.5F, (var6.stepSound.getStepVolume() + 1.0F) / 8.0F, var6.stepSound.getStepPitch() * 0.5F);
                    }
