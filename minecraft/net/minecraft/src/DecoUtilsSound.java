...
public class DecoUtilsSound {
	...
	public static boolean playCustomSoundForBlockConvert(Minecraft mc, World world, int x, int y, int z, Block block, int meta) {
		...
		if (block instanceof BlockLog || block instanceof FCBlockLogDamaged || block instanceof FCBlockLogSpike || block.blockID == DecoDefs.cherryLog.blockID || block.blockID == FCBetterThanWolves.fcBloodWood.blockID || block.blockID == DecoDefs.bloodLog.blockID) {
			if (DecoManager.getNewSoundsInstalled()) {
				customSound = true;
