...
public class AddonTileEntityBarrelFilling extends TileEntity /*implements FCITileEntityDataPacketHandler*/ {
	private ItemStack containedItems;
	
	@Override
	public void updateEntity() {
		if (containedItems != null && containedItems.stackSize >= 8) {
			((AddonBlockBarrelFilling) Block.blocksList[this.worldObj.getBlockId(this.xCoord, this.yCoord, this.zCoord)]).dropBlockAsItem(this.worldObj, this.xCoord, this.yCoord, this.zCoord, this.worldObj.getBlockMetadata(this.xCoord, this.yCoord, this.zCoord), 0);
	...
	public int attemptToAddItems(ItemStack stack) {
		...
		if (!AddonBlockBarrelFilling.isValidItemForFill(stack))
			return 0;
