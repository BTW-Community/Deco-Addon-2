...
public class DecoUtilsBlock {
	...
	public static boolean canFenceConnect(IBlockAccess blockAccess, int x, int y, int z, int facing, Block thisBlock) {
		...
		boolean isGlass = block != null && blockID == Block.glass.blockID || blockID == DecoDefs.glassStained.blockID;
	...
	public static boolean getWallConnectionAboveException(IBlockAccess blockAccess, int x, int y, int z, Block blockAbove) {
		...
		else if (blockAbove instanceof DecoBlockLantern ||
				blockAbove instanceof FCBlockStake ||
				blockAbove instanceof BlockSkull ||
				blockAbove instanceof FCBlockLogSpike) {
			return meta == 1;
	...
	public static boolean isFence(int blockID, int metadata) {
		return blockID == Block.fence.blockID || blockID == Block.netherFence.blockID || ((Block.blocksList[blockID] instanceof DecoBlockSidingAndCornerAndDecorative && metadata == 14) && !(Block.blocksList[blockID] instanceof DecoBlockSidingAndCornerDecorativeWall) && ((DecoBlockSidingAndCornerAndDecorative) Block.blocksList[blockID]).isFenceFromMetadata(metadata));
	}
	
	public static boolean isFenceGate(int blockID) {
		return blockID == Block.fenceGate.blockID || blockID == DecoDefs.gateBirch.blockID || blockID == DecoDefs.gateBlood.blockID || blockID == DecoDefs.gateJungle.blockID || blockID == DecoDefs.gateSpruce.blockID || blockID == DecoDefs.gateCherry.blockID;
	}
	...
	public static int getDamagedLogFromMetadata(int meta) {
		...
		case 1:
			return DecoDefs.logDamagedSpruce.blockID;
		case 2:
			return DecoDefs.logDamagedBirch.blockID;
		case 3:
	...
	public static double getFluidDripOffsetForBlockType(int blockID, int metadata) {
		...
		if (block instanceof DecoBlockTrapDoor) {
