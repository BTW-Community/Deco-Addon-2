...
public class DecoTileEntityHopper extends FCTileEntityHopper {
                    ...
                    else
                    {
                        ...
                        if (tileEntityBelow != null && tileEntityBelow instanceof IInventory)
                        {
                            ...
                            if (idBelow != Block.furnaceIdle.blockID && idBelow != Block.furnaceBurning.blockID)
                            {
                                ...
                                {
                                    var14 = 17;
                                    int var16 = ((DecoTileEntityHopper)tileEntityBelow).m_iFilterItemID;
                            ...
                            if (var15)
                            {
                                ...
                                {
                                	if (idBelow == DecoDefs.barrelFilling.blockID) {
                                		System.out.println("Barrel filling found");
                                    	//Makes sure barrel is facing up
                                    	if (DecoBlockBarrelFilling.isValidItemForFill(itemStackToEject)) {
                        ...
                        else if (blockBelow instanceof DecoBlockBarrelEmpty)
                        {
                        	int metadataBelow = this.worldObj.getBlockMetadata(x, yBelow, z);
                        	int newMeta = DecoBlockBarrelFilling.getMetadataFromBarrel(idBelow, metadataBelow);
                        	
                        	//Makes sure barrel is facing up
                        	if (DecoDefs.barrelEmpty.GetFacing(metadataBelow) == 0 && DecoBlockBarrelFilling.isValidItemForFill(itemStackToEject)) {
                        		this.worldObj.setBlock(x, yBelow, z, DecoDefs.barrelFilling.blockID, newMeta, 2);
    ...
    private boolean AttemptToEjectXPIntoHopper(int var1, int var2, int var3)
    {
        ...
        DecoTileEntityHopper var5 = (DecoTileEntityHopper)this.worldObj.getBlockTileEntity(var1, var2, var3);
    ...
    private int ejectItemIntoBarrel(ItemStack stack, int slot, int x, int y, int z) {
    	DecoTileEntityBarrelFilling barrelTile = (DecoTileEntityBarrelFilling) this.worldObj.getBlockTileEntity(x, y, z);
