...
public class DecoUtilsSound {
	...
	public static boolean playCustomSoundForBlockBreak(Minecraft mc, World world, int x, int y, int z, Block block, int meta) {
		...
		if (block.blockID == FCBetterThanWolves.fcAestheticOpaque.blockID && meta == 15) {
			if (DecoManager.getNewSoundsInstalled()) {
				customSound = true;
				playBlockSound(mc, x, y, z, DecoDefs.stepSoundBone, true);
			}
			else
				customSound = false;
		}
	...
	public static boolean playCustomSoundForBlockConvert(Minecraft mc, World world, int x, int y, int z, Block block, int meta) {
		...
		if (block instanceof BlockLog || block instanceof FCBlockLogDamaged || block instanceof FCBlockLogSpike || block.blockID == DecoDefs.cherryLog.blockID || block.blockID == FCBetterThanWolves.fcBloodWood.blockID || block.blockID == DecoDefs.bloodLog.blockID || block instanceof DecoBlockStem) {
			if (DecoManager.getNewSoundsInstalled()) {
				customSound = true;
